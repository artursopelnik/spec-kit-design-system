...
- `scripts/python/design.py` (single ~1900-line file; `scripts/bash/ds.sh` is a shim that finds an interpreter and forwards): all mechanism. Subcommands: `gate`, `query`, `principles`, `context`, `workflow`, `rfc`, `ledger`. Each emits exactly **one compact JSON object on stdout**; failures are JSON describing why, not non-zero exits, so command bodies can degrade instead of crash.
