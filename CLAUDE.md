...
- **Workflow position is derived from artifacts, no state file** (`workflow_status`): `[NEEDS CLARIFICATION]` in spec.md, `tasks.md` unticked boxes, `## Validation round N` headings, `- [ ] DS-F-nnn` finding checkboxes and a `## Verification` section in `design-system.md`. Every phase needs an artifact — `verify` was derived from the validate row instead, so it reported itself done and `next` skipped it. Those formats are a load-bearing contract with `commands/speckit.design.validate.md`. A round with ticked-off findings is not "clean". Rounds bounded by `max_validation_rounds` (2): round 1 is a full review, round 2 checks only the fixes, and unchecked fixes after the last round are `next: stop`, never another `validate`. The clean round writes `## Verification` in the same pass; `verify` survives as a rule only so a clean round that forgot it is visible.
- **`scan` states, it does not judge.** Raw values, contract token names the system lacks (`unknown_tokens`) and contract tokens seen nowhere in code (`tokens_not_seen`) are reported for the validate pass to turn into findings; `validation.theme_globs` exempts the files that define tokens. Only names whose first segment is a real token group count as token mentions, so paths and props are not misread.
