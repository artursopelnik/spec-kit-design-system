---
description: "Check the implementation independently against the spec, the guidelines and the design system, and raise findings that feed back into fixing"
---

# Design System Validate

Check what was actually built against what the spec, the guidelines and the ladder committed to. This runs as an **`after_implement` hook** and is the checker half of the workflow: the agent that wrote the code does not also get to decide it is correct.
...
Parse the gate for `FEATURE_DIR`, `DESIGN_DOC`, `IMPL_PLAN`, `CONFIG`, `CAPABILITIES`, `GUIDELINES_SOURCE`, `GUIDELINES_DISABLED`; the status for `validation_rounds_used`, `max_validation_rounds` and `may_validate_again`.
...
### 3. Check the guidelines in force

```
.specify/extensions/design/scripts/bash/ds.sh guidelines --applies-to <kinds from the spec> --json
```
...
Check against the code, not against the spec's claims. The spec saying `GL-FOCUS-VISIBLE` applies is not evidence that focus is visible.

Report a guideline finding under its id, so it is traceable to the guideline rather than to your phrasing of it. An id listed in `GUIDELINES_DISABLED` is not checked; note that it was skipped, rather than passing over it silently.
...
Guidelines source: `adapter`. Surface kinds: interactive, layout. Tests: 48 passed, 1 failed.
...
- [ ] DS-F-003 **violation** · Filter row · GL-FOCUS-VISIBLE: `outline: none` with no
...
- **violation**: contradicts a `DS-` requirement, a guideline in force, or a recorded ladder decision
...
- [ ] Every applicable guideline was checked against the code using its `verify` step, and skipped ids are named
